...
class QGpkg_qgis(QGpkg):
    ...
    def write(self, project_path):
        ...
        if not xmltree:
            self.log(
                logging.ERROR, u"Couldn't read project (wrong file format)")
            return None
        ...
        for layer in projectlayers:
            if layer.find("provider").text != "ogr":
                continue
            ...
            for path in sources:
                if self.database_connect(path):
                    if self.check_gpkg(path) and not gpkg_found:
                        ...
                        self.log(
                            logging.ERROR, u"The project uses layers "
        ...
        images = []
        for layout in root.findall("Layouts"):
            for lay in layout:
        ...
        self.c.execute('SELECT count(1) FROM qgis_projects WHERE name=?',
                       (project_name,))
        if self.c.fetchone()[0] == 0:
            self.c.execute('INSERT INTO qgis_projects VALUES (?,?)',
                           (project_name, project_xml))
    ...
    def read(self, gpkg_path):
            ...
            for lay in layout:
                for layout_picture in lay.findall("LayoutItem"):
                    if "file" in layout_picture.attrib:
                        ...
                        self.log(logging.DEBUG,
                                 u"External image %s found." % os.path.basename(
                                     layout_picture.attrib['file']))

        # and the image will be saved in the same folder as the project
        if images:
            self.c.execute(
                "SELECT name, mime_type, content FROM qgis_resources")
            images = self.c.fetchall()
            for img in images:
                img_name, mime_type, blob = img
                img_path = os.path.join(tmp_folder, os.path.basename(img_name))
        ...
        self.log(
            logging.DEBUG, u"Temporary project written: %s" % project_path)
        return project_path
