...
class QGpkg_owc (QGpkg):
    ...
    def write(self, project_path):
        self.log(logging.ERROR,
                 u"Sorry, but it appears that writing into this geopackage "
                 "extension was not implemented yet!")
        return
    ...
    def loadOWCLayer(self, gpkg_path, entry_elem):
        """Parses layer information from an entry, on OWC_context, and uses it
        to load and style the layer.
        ...
        # TODO: make offering more general, to support other types of data
        # formats (e.g.: wms)
        ...
        if offering_elem is not None:
            ...
            if content_elem is None:
                self.log(logging.ERROR,
                         u"Failed to content '" + name + "' layer!")
                return
            ...
            layer.setName(title_elem.text)
            if layer is None or not layer.isValid():
                self.log(logging.ERROR,
                         u"Layer '" + name + "' failed to load!")
                return
            ...
            if visibility is None:
                self.log(
                    logging.ERROR,
                    u"Failed to read visibility for '" + name + "' layer!")
                return

            QgsProject.instance().layerTreeRoot().findLayer(
                layer.id()).setItemVisibilityChecked(
                    visibility.lower() == 'true')
    ...
    def loadOWCStyle(self, style_elem, layer_title):
        ...
        if type != "application/sld+xml":
            self._log(
                logging.ERROR,
                u"Currently we only support styles in sld/xml format.")
            return
    ...
    def loadLayer(self, gpkg_path, layername, title):
        ...
        return iface.addVectorLayer(
            gpkg_path + "|layername=" + layername, title, "ogr")
    ...
    def loadStyle(self, style_name, table_name, given_name):
        ...
        Args:
            ...
            table_name: The name of the table where the style is stored
            (shouldn't it be a convention?).
        ...
        try:
            ...
            self.log(
                logging.ERROR, u"Could not find style " + style_name)
            return
        ...
        layerList = QgsProject.instance().mapLayersByName(given_name)

        if len(layerList) == 0:
            self.log(logging.ERROR,
                     u"We could not find a loaded layer "
